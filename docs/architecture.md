...
Local Live Mode uses profile `local-live`: same-origin SPA + API + WebSocket, PostgreSQL on localhost, LAN join URLs, assessment packages and an optional Ubuntu hotspot. See [ADR 0006](adr/0006-local-live-mode.md) and [local-live.md](local-live.md). Redis and RabbitMQ stay out of Local Live.
